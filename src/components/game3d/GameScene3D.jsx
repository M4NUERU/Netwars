...
function CameraRig() {
    useFrame((state) => {
        state.camera.position.lerp(new THREE.Vector3(state.mouse.x * 4, 12 + state.mouse.y * 2, 26), 0.05);
...
export default function GameScene3D({ game, currentPlayer, onPlayCard, onHoverCard, onHoverEnd, onEndTurn, onReboot, log }) {
    ...
    return (
        <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
            <Canvas shadows camera={{ position: [0, 12, 26], fov: 45 }}>
                ...
                {/* 3D Terminal Log */}
                <Html position={[14, 5, 6]} transform rotation={[0, -0.4, 0]}>
                ...
                {/* 3D Action Buttons */}
                <Html position={[10, 0, 14]} transform rotation={[-Math.PI / 4, 0, 0]}>
